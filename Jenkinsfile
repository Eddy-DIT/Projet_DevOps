        ...
        stage('Upload Jar to nexus'){
            steps{
                ...
                nexusArtifactUploader artifacts: [
                    [
                        artifactId: 'projet-devops',
                        classifier: '',
                        file: 'target/projet-devops-0.0.1-SNAPSHOT.jar',
                        type: 'jar'
                    ]
                ],
                credentialsId: 'nexus',
                groupId: 'projet-devops-dit',
                nexusUrl: '${NEXUS_URL}',
                nexusVersion: 'nexus3',
                protocol: 'http',
                repository: 'projet-devops',
             ...
             stage('Deploy to cluster') {
                  steps {
                    script {
                       sshagent(['demodeploy']) {
                           sh'ssh -oStrictHostKeyChecking=no root@10.0.0.20 kubectl delete deploy --ignore-not-found=true 
projet-devops-app'
